...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  channel = await connection.createChannel();
    ...
    const cancelBookingQueue = "pearl-fix/booking/dentist/cancel";
    await channel.assertQueue(cancelBookingQueue, { durable: true });

    const { bookingId, dentistEmail } = req.body;
    console.log(bookingId);
    console.log(dentistEmail);
    channel.sendToQueue(cancelBookingQueue, Buffer.from(JSON.stringify({bookingId, dentistEmail})), {
      persistent: true,
    });

    const responseQueue = "pearl-fix/booking/dentist/cancel/authenticate";
